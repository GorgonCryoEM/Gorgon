...
namespace wustl_mm {
		...
		void CAlphaRenderer::DrawRibbonModel(int subSceneIndex, bool selectEnabled) {
			...
			//else if (subSceneIndex == 2){
				...
				for (unsigned int i = 0; i < loops.size(); ++i){
					...
					cout << "currentSecel.atomHashes.size(): " << currentSecel.atomHashes.size() << endl;
					if(currentSecel.atomHashes.size() > 1){
						...
						vector<Vector3DFloat> renderingPoints((ptsize)*NUM_SLICES);
						vector<Vector3DFloat> renderingNormals((ptsize)*NUM_SLICES);
						...
						cout << "about to draw the tube with renderingPoints.size(): " << renderingPoints.size() << endl;
						DrawTube(renderingPoints, renderingNormals, interpolatedPoints.size() - 1, NUM_SLICES);
						cout << "about to draw a sphere, even though I shouldn't" << endl;
						DrawSphere(interpolatedPoints[interpolatedPoints.size() - 1], LOOP_RADIUS);
					}
	...
	void CAlphaRenderer::DrawTube(std::vector<Vector3DFloat> points, std::vector<Vector3DFloat> normals, int stacks, int slices){
		cout << "about to draw a tube in the drawtube method" << endl;
	...
	vector<Vector3DFloat> CAlphaRenderer::InterpolateLoopPoints(std::vector<Vector3DFloat> points, Vector3DFloat previous, Vector3DFloat next, int NUM_SECTIONS){
		...
		//vector<Vector3DFloat> interpolatedPoints((pointstemp.size()-1)*(NUM_SECTIONS));
		...
		for(unsigned int i = 0; i < points.size()-1; ++i){
			...
			//interpolatedPoints[i*(NUM_SECTIONS)] = pointstemp[i];
			interpolatedPoints[i*(NUM_SEGMENTS)] = pointstemp[i];
			//for (int sect = 1; sect < NUM_SECTIONS; ++sect){
			//double tsect = ((double)sect)/((double)NUM_SECTIONS);
			//	interpolatedPoints[i*(NUM_SECTIONS) + sect] = curve.getPos(tsect);
			//}
			for (int sect = 1; sect < NUM_SEGMENTS; ++sect){
				double tsect = ((double)sect)/((double)NUM_SEGMENTS);
