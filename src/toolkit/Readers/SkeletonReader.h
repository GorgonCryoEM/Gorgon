...
namespace GraphMatch {
    ...
    inline Graph * readFile(string skeletonFile, string helixFile, string sseFile, string sheetFile) {
        ...
        for(int x = 0; x < skel->getSizeX(); x++) {
            point[0] = skel->getOriginX() + x * skel->getSpacingX();
            for(int y = 0; y < skel->getSizeY(); y++) {
                point[1] = skel->getOriginY() + y * skel->getSpacingY();
                for(int z = 0; z < skel->getSizeZ(); z++) {
                    point[2] = skel->getOriginZ() + z * skel->getSpacingZ();
...
//        #endif
        ...
        sheetClusters->setOrigin(skel->getOriginX(), skel->getOriginY(), skel->getOriginZ() );
        sheetClusters->setSpacing(skel->getSpacingX(), skel->getSpacingY(), skel->getSpacingZ() );
        ...
        for(unsigned int i = 0; i < (int)helixes.size(); i++) {
            if(helixes[i]->type == GRAPHEDGE_HELIX) {
                ...
                findCornerCellsInSheet(skel, &paintedVol, helixes, i);
        ...
        for(int i = 0; i < (int)helixes.size(); i++) {
            ...
            for(int j = 0; j < (int)helixes[i]->cornerCells.size(); j++) {
                ...
                findSizes(i, j, helixes, skel, &paintedVol, graph);
            }
        }
    ...
    inline int isSkeletonSheet(const Volume &vol, int ox, int oy, int oz )
    {
        int cn = 12 ;
        int nx, ny, nz ;
    ...
    inline void findCornerCellsInSheet(Volume * vol, Volume * paintedVol, vector<Shape*> & helixes, int sheetId) {
    ...
    inline void readSheetFile(string sheetFile, vector<Shape*> & helixes){
...
//        #endif
    ...
    inline void findPaths(Graph * graph) {
    ...
    inline void findPath(int startIx, int endIx, vector<vector<Vec3I> > nodes, Volume * maskVol, Graph * graph, bool eraseMask) {
        // erase any old path
