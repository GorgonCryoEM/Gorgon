...
{
public:
	vector<Vec3F> Deform(vector<Vec3F>&, vector<Vec3F>&, vector<Vec3F>&, int);
	CurveDeformer(void);
	~CurveDeformer(void);
...
private:
	matrix<float> constructA(std::vector<Vec3F>&, int, int);
	matrix<float> constructB(std::vector<Vec3F>&, int);
	//vector<Vec3F> solveSystemQR(matrix<float>&, matrix<float>&, MatrixFloat&);
	vector<Vec3F> solveSystemQR(matrix<float>&, matrix<float>&, TNT::Matrix<float>&);
	vector<Vec3F> solveSystemLU(matrix<float>&, matrix<float>&, TNT::Matrix<float>&);
	vector<Vec3F> solveSystemCholesky(matrix<float>&, matrix<float>&, TNT::Matrix<float>&);
	matrix<float> constructAi(vector<Vec3F> &, int, int);
	matrix<float> constructSi(vector<Vec3F> &, int);
	vector<Vec3F> calculateLaplacians(vector<Vec3F>&);
	matrix<float> generateLaplacianCoefs(vector<Vec3F>&, int);
	void addHandleConstraints(matrix<float> &, matrix<float> &, vector<Vec3F>&, vector<Vec3F>&);
...
inline CurveDeformer::CurveDeformer(void)
{
...
inline vector<Vec3F> CurveDeformer::Deform(vector<Vec3F>& originalLocations, vector<Vec3F>& hardHandles, vector<Vec3F>& softHandles, int numNeighbors)
{
	...
	//vector<Vec3F> resultCH = solveSystemCholesky(A,B, flat);
	vector<Vec3F> resultLU = solveSystemLU(A,B, flat);
	//vector<Vec3F> resultQR = solveSystemQR(A,B, flat);
...
inline matrix<float> CurveDeformer::constructA(vector<Vec3F>& origPoints, int extraConstraints, int numNeighbors)
{
	...
	vector<Vec3F> laplacians = calculateLaplacians(origPoints);

	for(int i = 1; i < numPoints-1; ++i)
	{
        // A below is C in slides; S below is D in slides
...
inline matrix<float> CurveDeformer::constructB(vector<Vec3F>& points, int extraConstraints)
{
...
inline matrix<float> CurveDeformer::generateLaplacianCoefs(vector<Vec3F> & points, int numHandles)
{
...
inline vector<Vec3F> CurveDeformer::calculateLaplacians(vector<Vec3F> & points)
{
	//storing them as Vector3D
	vector<Vec3F> result(points.size());

	//laplacian is centroid of neighbors subtracted from original point
	for(int i = 1; i < points.size()-1; ++i)
	{
		result[i] = Vec3F(points[i-1]*(-0.5) + points[i+1]*(-0.5) + points[i]);
	}
	return result;
}

inline matrix<float> CurveDeformer::constructAi(std::vector<Vec3F> & points, int index, int numNeighbors)
{
...
inline matrix<float> CurveDeformer::constructSi(std::vector<Vec3F> & laplacians, int index)
{
...
inline void CurveDeformer::addHandleConstraints(matrix<float> & A, matrix<float> & B, vector<Vec3F>& hardHandles, vector<Vec3F>& softHandles)
{
...
inline void CurveDeformer::setLaplacainW(float in){
	laplacianWeight = in;
}
inline void CurveDeformer::setHardHandleW(float in){
	hardHandleWeight = in;
}
inline void CurveDeformer::setSoftHandleW(float in){
	softHandleWeight = in;
}


//------------------------------Linear System Solvers------------------------------------//
inline vector<Vec3F> CurveDeformer::solveSystemQR(matrix<float>& A, matrix<float>& B, TNT::Matrix<float>& flat)
{
	bool singular = false;
	...
	flat = solved;
	vector<Vec3F> result(solved.num_rows()/3);

	//fill the transformed vertices back into a vector
	for(int i = 0; i < solved.num_rows(); i = i+3)
	{
		result[i/3] = Vec3F(solved(i+1,1), solved(i+2,1), solved(i+3,1));
	}
...
inline vector<Vec3F> CurveDeformer::solveSystemLU(matrix<float>& A, matrix<float>& B, TNT::Matrix<float>& flat)
{
	bool singular = false;
...
inline vector<Vec3F> CurveDeformer::solveSystemCholesky(matrix<float>& A, matrix<float>& B, TNT::Matrix<float>& flat)
{
	bool singular = false;
	//cout << "started two mults" << endl;
	matrix<float> AABoost = prod(trans(A),A);
...
inline matrix<float> CurveDeformer::InvertMatrix (const matrix<float>& input) {
...
inline boost::numeric::ublas::matrix<float> CurveDeformer::gjinverse(const boost::numeric::ublas::matrix<float> &m, bool &singular)
 {
     using namespace boost::numeric::ublas;
