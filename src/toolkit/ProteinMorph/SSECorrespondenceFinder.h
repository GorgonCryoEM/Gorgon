...
namespace wustl_mm {
	namespace Protein_Morph {
		...
		public:
			...
			MatrixFloat GetTransform(vector<Vec3F>, vector<Vec3F>, int sampleCount);
		...
		private:
			...
			inline void GetFeatureAngles(float & joint1, float & joint2, float & dihedral, Vec3F p, Vec3F pDir, Vec3F q, Vec3F qDir);
		...
		inline SSECorrespondenceFinder::SSECorrespondenceFinder() {
			featureList1.clear();
			featureList2.clear();
		...
		inline void SSECorrespondenceFinder::InitializeFeatures(vector<SSECorrespondenceFeature> featureList1, vector<SSECorrespondenceFeature> featureList2) {
		...
		inline long long SSECorrespondenceFinder::AtomToHash(char chainId, int seqNo) {
			return (long long)chainId * 100000 + (long long)seqNo;
		}

		inline long long SSECorrespondenceFinder::CorrespondenceToHash(int h1, int h2) {
			return (long long)h1 * 100000 + (long long)h2;
		}

		inline void SSECorrespondenceFinder::HashToCorrespondence(long long hash, int & h1, int & h2) {
			h2 = hash % 100000;
			h1 = hash / 100000;
		}

		inline void SSECorrespondenceFinder::PrintGroundTruth(vector<PDBHelix> & h1, vector<PDBHelix> & h2, bool multipleSearch) {
		...
		/**vector<PDBHelix> SSECorrespondenceFinder::ReadHelixPositions(string fileName) {
            ...
            while(!feof(fin))
            {
                ...
                if(token.compare("HELIX ") == 0) {
                    PDBHelix helix = wustl_mm::GraphMatch::PDBHelix(lineStr);
                    helixAtomLocs.clear();
		...
		inline void SSECorrespondenceFinder::InitializeFeaturesFromPDBFiles(string file1, string file2, bool multipleSearch) {
			vector<PDBHelix> helices1 = PDBReader::ReadHelixPositions(file1);
			vector<PDBHelix> helices2 = PDBReader::ReadHelixPositions(file2);
			//vector<PDBHelix> helices1 = ReadHelixPositions(file1);
			//vector<PDBHelix> helices2 = ReadHelixPositions(file2);
		...
		inline float SSECorrespondenceFinder::GetFeatureCompatibilityScore(SSECorrespondenceFeature feature1, SSECorrespondenceFeature feature2) {
			//return pow(feature1.Length() - feature2.Length(), 2);
			return abs(feature1.Length() - feature2.Length());
		}

		inline float SSECorrespondenceFinder::GetFeaturePairRigidityScore(SSECorrespondenceFeature p1, SSECorrespondenceFeature q1, bool isForward1, SSECorrespondenceFeature p2, SSECorrespondenceFeature q2, bool isForward2, bool useDirection) {
			float pJoint1, pJoint2, pDihedral, qJoint1, qJoint2, qDihedral;
			Vec3F q1p, q1Dir, q2p, q2Dir;
		...
		inline void SSECorrespondenceFinder::GetFeatureAngles(float & joint1, float & joint2, float & dihedral, SSECorrespondenceFeature feature1, SSECorrespondenceFeature feature2) {
			Vec3F f1 = Vec3F::normalize(feature1.GetFeatureVector());
			Vec3F f2 = Vec3F::normalize(feature2.GetFeatureVector());
			Vec3F c = Vec3F::normalize(feature1.GetCentroid() - feature2.GetCentroid());
			joint1 = acos(f1 * c);
			joint1 = min(joint1, (float)(PI-joint1));
			joint2 = acos(f2 * c);
			joint2 = min(joint2, (float)(PI-joint2));

			Vec3F f1n = Vec3F::normalize(f1 - c * (f1*c));
			Vec3F f2n = Vec3F::normalize(f2 - c * (f2*c));
			dihedral = acos(f1n * f2n);
			dihedral = min(dihedral, (float)(PI-dihedral));
		}

		inline void SSECorrespondenceFinder::GetFeatureAngles(float & joint1, float & joint2, float & dihedral, Vec3F p, Vec3F pDir, Vec3F q, Vec3F qDir) {

			Vec3F f1 = Vec3F::normalize(pDir);
			Vec3F f2 = Vec3F::normalize(qDir);
			Vec3F c = Vec3F::normalize((p+p+pDir)*0.5 - (q+q+qDir)*0.5);
			joint1 = acos(f1 * c);
			joint2 = acos(f2 * c);
		...
		inline vector< vector<float> > SSECorrespondenceFinder::GetAllFeatureCompatibilityScores() {
		...
		inline vector<SSECorrespondenceNode> SSECorrespondenceFinder::GetAllNodes(vector< vector<float> > & featureCompatibilityScores, bool splitOnDirection) {
		...
		inline vector< vector<float> > SSECorrespondenceFinder::GetAllNodePairCompatibilityScores(vector<SSECorrespondenceNode> & nodes, bool useDirection) {
			vector< vector<float> > scores;
		...
		inline bool SSECorrespondenceFinder::IsFeaturePairRigid(SSECorrespondenceNode n1, SSECorrespondenceNode n2, bool useDirection) {
		...
		inline bool SSECorrespondenceFinder::IsFeaturePairRigid(SSECorrespondenceFeature p1, SSECorrespondenceFeature q1, bool isForward1, SSECorrespondenceFeature p2, SSECorrespondenceFeature q2, bool isForward2, bool useDirection) {
			float pJoint1, pJoint2, pDihedral, qJoint1, qJoint2, qDihedral;
			bool retVal = true;						
		...
		inline vector< vector < vector<SSECorrespondenceNode> > > SSECorrespondenceFinder::GetAStarCliqueBasedFeatureCorrespondence(bool printOutput, bool useDirection) {
		...
		inline vector< vector < vector<SSECorrespondenceNode> > > SSECorrespondenceFinder::GetAStarTriangleBasedFeatureCorrespondence(bool printOutput, bool useDirection, int smallestCliqueSize) {
			vector< vector < vector<SSECorrespondenceNode> > > correspondence;
			vector< vector<float> > featureCompatibilityScores = GetAllFeatureCompatibilityScores();
			vector<SSECorrespondenceNode> nodes = GetAllNodes(featureCompatibilityScores, useDirection);
		...
		inline vector< vector < vector<SSECorrespondenceNode> > > SSECorrespondenceFinder::GetAStarTriangleBasedCliqueDistanceFeatureCorrespondence(bool printOutput, bool useDirection, int smallestCliqueSize) {			
		...
		inline void SSECorrespondenceFinder::GetCliqueFeatures(vector< set<unsigned long long> > & cliqueFeaturesP, vector< set<unsigned long long> > &cliqueFeaturesQ, vector< set<unsigned long long> > &allCliques, vector<SSECorrespondenceNode> &nodes) {
		...
		inline vector< vector < vector<SSECorrespondenceNode> > > SSECorrespondenceFinder::GetCliquesFirstAStarTriangleBasedCliqueDistanceFeatureCorrespondence(bool printOutput, bool useDirection, int smallestCliqueSize, int levelSizeTolerence) {			
			TimeManager tm;
			tm.PushCurrentTime();
			...
			//printf("(*%d cliques found ", strlen(allCliques));
		...
		inline vector < vector<SSECorrespondenceNode> > SSECorrespondenceFinder::GetValenceBasedFeatureCorrespondence(bool printOutput, bool useDirection) {
			vector< vector<float> > featureCompatibilityScores = GetAllFeatureCompatibilityScores();
			vector<SSECorrespondenceNode> nodes = GetAllNodes(featureCompatibilityScores, useDirection);
			vector< vector<float> > pairCompatibility = GetAllNodePairCompatibilityScores(nodes, useDirection);
		...
		inline vector < vector<SSECorrespondenceNode> > SSECorrespondenceFinder::GetValenceTriangleBasedFeatureCorrespondence(bool printOutput, bool useDirection, int smallestCliqueSize) {
			TimeManager m;
			m.PushCurrentTime();
			m.PushCurrentTime();
			vector< vector<float> > featureCompatibilityScores = GetAllFeatureCompatibilityScores();
		...
		inline vector < vector < vector<SSECorrespondenceNode> > > SSECorrespondenceFinder::GetMultiCorrespondence(int method, bool useDirection, int smallestCliqueSize) {
			maxSolutionCount = 1;
		...
		inline void SSECorrespondenceFinder::PrintFeatureListsMathematica() {
			Vec3F pt0, pt1;
			printf("fl1 = {\n");
			for(unsigned int i = 0; i < featureList1.size(); i++) {
		...
		inline GraphBase<unsigned int, bool> SSECorrespondenceFinder::CreateGraph(vector< vector<float> > & featureCompatibilityScores, vector<SSECorrespondenceNode> & nodes, bool useDirection) {
			GraphBase<unsigned int, bool> parentGraph;
		...
		inline void SSECorrespondenceFinder::PrintTimes() {
		...
		inline MatrixFloat SSECorrespondenceFinder::GetTransform(vector<SSECorrespondenceNode> cluster, int sampleCount) {
			if(cluster.size() <= 1) {
			...
			for(unsigned int i = 0; i < cluster.size(); i++) {
				...
				Vec3F p1trans = p1.transform(result);
				Vec3F p2trans = p2.transform(result);
		...
		inline MatrixFloat SSECorrespondenceFinder::GetTransform(vector<Vec3F> originalLocs, vector<Vec3F> newLocs, int sampleCount) {
