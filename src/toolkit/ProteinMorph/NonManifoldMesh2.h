...
//namespace wustl_mm {
	namespace Protein_Morph {
		...
		public:
			...
			NonManifoldMeshVertex2(Vec3F position) {
		...
		template <class TVertex, class TEdge, class TFace> Vec3F NonManifoldMesh2<TVertex, TEdge, TFace>::GetVertexNormal(unsigned long long vertexHash) {
		...
		template <class TVertex, class TEdge, class TFace> Vec3F NonManifoldMesh2<TVertex, TEdge, TFace>::GetFaceNormal(unsigned long long faceHash) {
		...
		/**template <class TVertex, class TEdge, class TFace> void NonManifoldMesh2<TVertex, TEdge, TFace>::Draw(bool drawSurfaces, bool drawLines, bool drawPoints, bool annotateSurfaces, bool annotateLines, bool annotatePoints, bool disableSurfaceLighting, bool disableCurveLighting, bool disablePointLighting, int lineThickness) {
			...
			if(drawSurfaces) {
				...
				// face::
				//std::map<unsigned long long, NonManifoldMeshFace2<TFace> >iterator i;
				for(std::map< unsigned long long, NonManifoldMeshFace2< TFace > >iterator i = faces.begin(); i != faces.end(); i++) {					
					if(annotateSurfaces) {
						glLoadName(i->first);
					}
					glBegin(GL_POLYGON);
