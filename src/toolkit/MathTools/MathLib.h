...
namespace wustl_mm {
	namespace MathTools {
		...
		inline void MathLib::EigenAnalysis(EigenVectorsAndValues2D & eigenInformation) {
			return mathWrapper->EigenAnalysis(eigenInformation);
		}

		inline void MathLib::EigenAnalysis(EigenVectorsAndValues3D & eigenInformation) {
			#ifdef USE_MATLAB
