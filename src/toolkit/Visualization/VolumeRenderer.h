...
namespace Visualization {
    ...
    Volume * VolumeRenderer::PerformGrayscaleSkeletonizationAbeysinghe2008(double startDensity, int stepCount, int minCurveSize, int minSurfaceSize, int curveRadius, int surfaceRadius, int skeletonRadius) {
        ...
        if(!isZero(stepSize)) {
            ...
            Volume * outputVol = skeletonizer->PerformImmersionSkeletonizationAndPruning(GetVolume(), NULL, startDensity, Volume::getMax(), stepSize, 0, 0, minCurveSize, minSurfaceSize, 0, 0, "", true, 1.0, DEFAULT_PRUNE_THRESHOLD, DEFAULT_PRUNE_THRESHOLD);
            delete skeletonizer;
            return outputVol;
        } else {
            return NULL;
