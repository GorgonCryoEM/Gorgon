...
class HistogramSliderWidget(HistogramWidget):
    ...
    def mouseCoordToValue(self, x, y):
        return self.minValue + (self.maxValue - self.minValue) * (
        float(x - self.verticalBorderSize) / float(self.width() - 2 * self.verticalBorderSize))

    def isMouseInLowerValue(self, x, y):
        valLoc = (self.width() - 2 * self.verticalBorderSize) * (self.values[0] - self.minValue) / (
    ...
    def isMouseInHigherValue(self, x, y):
        valLoc = (self.width() - 2 * self.verticalBorderSize) * (self.values[1] - self.minValue) / (
        self.maxValue - self.minValue)
        return QtCore.QRect(valLoc + self.sliderRadius, 0, self.sliderRadius * 2 + 1, self.height() - 1).contains(x, y)
    ...
    def resizeEvent(self, event):
        self.minGapBetweenSliders = self.minValue + (self.maxValue - self.minValue) * (
        float(10) / float(self.width() - 2 * self.verticalBorderSize))
