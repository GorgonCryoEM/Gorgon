...
public:
		...
		bool parallelWriteRawVFile(const char *filename);
		bool parallelWriteRawIVFile(const char *filename);
#endif
		...
		bool writeRawVFile(const char *filename);
		bool writeRawIVFile(const char *filename, unsigned int variable=0);
		...
		int getDataSize( unsigned int variable );
		...
		double getValueAt( unsigned int variable, int index);
