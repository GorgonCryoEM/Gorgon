...
#ifndef TEST_CPSC
   #define TEST_CPSC ATL_UCPSC
#endif
void TEST_CPSC(const int N, const SCALAR al, const TYPE *X, const int incX,
...
int DoTest(int N, TYPE *alpha0, int incX, int incY)
{
   ...
   good_cpsc(N, alpha, x, incX, y, incY);
   y = Yt + (npad SHIFT);
   if (incY < 1) y -= ((N-1)SHIFT) * incY;
   TEST_CPSC(N, alpha, x, incX, y, incY);
               ...
               int nalpha, TYPE *alp)
{
   ...
   if (iret == 0) fprintf(stdout, "ALL CPSC SANITY TESTS PASSED.\n\n");
   else fprintf(stdout, "%d of %d CPSC TESTS FAILED!!\n\n", iret, i);
   return(iret);
}
