                 ...
                 int alpha, int beta, char *cc, char *ccflags)
{
   ...
   if (fpin == NULL)
   {
      i = sprintf(ln, "make %cscalcase N=%d urout=%s fout=\"-f %s\" ",
                  pre, N, rout, fnam);
      if (cc) i += sprintf(ln+i, "UCC=\"%s\" ", cc);
      if (ccflags) i += sprintf(ln+i, "UCCFLAGS=\"%s\" ", ccflags);
      i += sprintf(ln+i, "opt=\"");
...
FILENODE *TstFile(char pre, FILENODE *fbase, int *N)
{
   ...
   for (fp=fbase; fp; fp = fn)
   {
      ...
      i += sprintf(ln+i, "\" ");
      if (fp->cc) i += sprintf(ln+i, "UCC=\"%s\" ", fp->cc);
      if (fp->ccflags) i += sprintf(ln+i, "UCCFLAGS=\"%s\" ", fp->ccflags);
      sprintf(ln+i, "\n");
                 ...
                 int n, int *ix, int *iy, int *ia, int *ib)
{
   ...
   for (i=0; i < n; i++)
   {
      if (ia[i] == alpha)
      {
         ...
         fprintf(fpout, "%s      %s(N, alpha, X, incx);\n",
                 ...
                 FILENODE **bp)
{
   ...
   fprintf(fpout, "\t$(CC) $(CCFLAGS) -D%s -o $@ -c %s.c\n", typ, ln);
   for (i=0; i < n; i++)
   {
 ...
 */
   ...
   fprintf(fpout, "\nvoid ATL_%cscal(const int N, const SCALAR alpha, TYPE *X, const int incX)\n{\n", pre);
   ...
   if (pre == 'c' || pre == 'z')
   {
      ...
      fprintf(fpout,
         "%s{\n", spc);
      fprintf(fpout,
         "%s   Mjoin(PATLU,scal)(N+N, *alpha, X, 1);\n", spc);
      fprintf(fpout,
         "%s   return;\n", spc);
      fprintf(fpout,
         "%s}\n", spc);
 ...
 */
{
   ...
   for (i=0; i < n; i++)
   {
      ...
      fpout = fopen(ln, "w");
      assert(fpout);
      fprintf(fpout, "#define ATL_USCAL %s\n\n", nam);
