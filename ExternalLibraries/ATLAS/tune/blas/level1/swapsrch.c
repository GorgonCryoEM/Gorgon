 ...
 */
   ...
   if (pre == 'c' || pre == 'z')
      fprintf(fpout, "void Mjoin(PATLU,swap)\n   (%s);\n", dargs);

   fprintf(fpout,
      "\nvoid ATL_%cswap(const int N, const TYPE *X, const int incX,\n", pre);
   ...
   if (pre == 'c' || pre == 'z')
   {
      ...
      fprintf(fpout,
         "%s{\n", spc);
      fprintf(fpout,
         "%s   Mjoin(PATLU,swap)(N+N, X, 1, Y, 1);\n", spc);
      fprintf(fpout,
         "%s   return;\n", spc);
      fprintf(fpout,
         "%s}\n", spc);
 ...
 */
{
   ...
   for (i=0; i < n; i++)
   {
      ...
      fpout = fopen(ln, "w");
      assert(fpout);
      fprintf(fpout, "#define ATL_USWAP %s\n\n", nam);
