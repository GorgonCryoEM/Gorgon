...
#ifndef TEST_SCAL
   #define TEST_SCAL ATL_SCAL
#endif

double DoTiming(int N, int nkflop, int cachesize, SCALAR alpha, int incX)
{
   void TEST_SCAL(const int N, const SCALAR alpha, TYPE *X, const int incX);
 ...
 */
{
   ...
   for (i=0; i < nrep; i++)
   {
      tims[i] = DoTiming(N, nkflop, cachesize, alpha, incX);
 ...
 */
{
   ...
   do
   {
      ...
      GetTimes(nrep-n, tims+n, N, nkflop, cachesize, alpha, incX);
               ...
               SCALAR alpha, int incX)
{
   ...
   for (i=0; i < nN; i++)
   {
      t0 = GetAvgMf(Ns[i], nkflop, cachesize, fout, alpha, incX, 3, 0.1);
...
main(int nargs, char **args)
{
   ...
   GetFlags(nargs, args, &nN, &Ns, &nkflops, &cachesize, &fout,
            &incX, al);
   DoTimings(nN, Ns, nkflops, cachesize, fout, alpha, incX);
   free(Ns);
   exit(0);
}
