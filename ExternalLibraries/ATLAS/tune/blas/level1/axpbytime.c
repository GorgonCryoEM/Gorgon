...
#ifndef TEST_AXPBY
   #define TEST_AXPBY ATL_AXPBY
#endif

double DoTiming(int N, int nkflop, int cachesize, SCALAR alpha, SCALAR beta,
                int incX, int incY)
{
   void TEST_AXPBY(const int N, const SCALAR alpha, const TYPE *X,
   ...
   {
      y = x + ii;
      TEST_AXPBY(N, alpha, x+ix, incX, beta, y+iy, incY);
      x = y + jj;
      if (x == stX) x = X;
   }
   t1 = time00() - t0;
 ...
 */
{
   ...
   for (i=0; i < nrep; i++)
   {
      tims[i] = DoTiming(N, nkflop, cachesize, alpha, beta, incX, incY);
 ...
 */
{
   ...
   do
   {
      ...
      GetTimes(nrep-n, tims+n, N, nkflop, cachesize, alpha, beta, incX, incY);
               ...
               SCALAR alpha, SCALAR beta, int incX, int incY)
{
   ...
   for (i=0; i < nN; i++)
   {
      t0 = GetAvgMf(Ns[i], nkflop, cachesize, fout, alpha, beta, incX, incY, 3, 0.1);
      #ifdef TREAL
         fprintf(stdout,
...
main(int nargs, char **args)
{
   ...
   GetFlags(nargs, args, &nN, &Ns, &nkflops, &cachesize, &fout,
            &incX, &incY, al, be);
   DoTimings(nN, Ns, nkflops, cachesize, fout, alpha, beta, incX, incY);
   free(Ns);
   exit(0);
}
