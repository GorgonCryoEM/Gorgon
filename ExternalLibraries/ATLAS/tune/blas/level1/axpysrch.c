 ...
 */
   ...
   if (pre == 'c' || pre == 'z')
      fprintf(fpout, "void Mjoin(PATLU,axpy)\n   (const int, const TYPE, const TYPE*, const int, TYPE*, const int);\n");

   fprintf(fpout, "\nvoid ATL_%caxpy(const int N, const SCALAR alpha, const TYPE *X, const int incX,\n", pre);
   ...
   if (pre == 'c' || pre == 'z')
   {
      ...
      fprintf(fpout,
         "%s{\n", spc);
      fprintf(fpout,
         "%s   Mjoin(PATLU,axpy)(N+N, *alpha, X, 1, Y, 1);\n", spc);
      fprintf(fpout,
         "%s   return;\n", spc);
      fprintf(fpout,
         "%s}\n", spc);
 ...
 */
{
   ...
   for (i=0; i < n; i++)
   {
      ...
      fpout = fopen(ln, "w");
      assert(fpout);
      fprintf(fpout, "#define ATL_UAXPY %s\n\n", nam);
